...
async def test_all_persons(es_client, make_get_request, redis_client, persons_index):
    ...
    page_size = 10
    await redis_client.flushall()
    cache_key = generate_cache_key(index='persons', query=None, page=1, page_size=page_size)
    ...
    response = await make_get_request(endpoint=f'{test_settings.person_router_prefix}/search', params=params)
    assert response.status == HTTPStatus.OK
    api_persons = response.body
    assert len(api_persons) == page_size
